   ...
   for (y=image_height; y; y--) {
	 ...
	 case 3:
	    if (compressed)
		  ...
		  goto error;
	       ...
	       if (!raw_tga_read8(bmp->line[yc], image_width, f))
		  goto error;
	    break;
	 ...
	 case 2:
	    if (bpp == 32) {
		  ...
		  if (!raw_tga_read32((unsigned int *)bmp->line[yc], image_width, f))
		     goto error;
	    }
	    else if (bpp == 24) {
	       if (compressed)
		  ...
		  if (!raw_tga_read24(bmp->line[yc], image_width, f))
		     goto error;
	    }
	    else {
	       if (compressed)
		  ...
		  if (!raw_tga_read16((unsigned short *)bmp->line[yc], image_width, f))
		     goto error;
	    }
	    break;
      }
